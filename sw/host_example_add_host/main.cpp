...
int main(int argc, char** argv) {
	...
	cout << "[STEP 2] Allocate Buffer in Global Memory (HOST[0] Native XRT Mode)" << endl;
	fflush( stdout );
        xrt::bo::flags flags = xrt::bo::flags::host_only;
	auto boIn  = xrt::bo(device, (size_t)DATA_SIZE, flags, krnl.group_id(1));
	auto boOut = xrt::bo(device, (size_t)DATA_SIZE, flags, krnl.group_id(2));
